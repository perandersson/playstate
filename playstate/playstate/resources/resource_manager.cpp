...
template<> playstate::ResourceManager* playstate::Singleton<playstate::ResourceManager>::gSingleton = NULL;

ResourceManager::ResourceManager(RenderSystem& renderSystem, IFileSystem& fileSystem) 
...
ResourceData* ResourceManager::GetResourceData(const std::string& path)
{
...
std::string ResourceManager::GetSuffixFromName(const std::string& name) const
{
...
ResourceData* ResourceManager::LoadResourceAndWait(const std::string& path, IResourceLoader* loader)
{
	std::auto_ptr<IFile> file = mFileSystem.OpenFile(path);
...
void ResourceManager::RegisterResourceType(IResourceLoader* resourceLoader, const std::string& suffix)
{
...
void ResourceManager::AddLoadRequest(const std::string& name, IResourceLoader* loader, ResourceData* data)
{
	data->Unloaded = false;
	data->Loading = true;
	data->DefaultResource = loader->GetDefaultResource();
...
void ResourceManager::GetAndClearLoadRequests(LoadRequests& target)
{
	if(mRequests.empty())
		return;
...
void ResourceManager::GetAndClearLoadResponses(LoadResponses& target)
{
	if(mResponses.empty())
		return;
...
void ResourceManager::UnloadResourceData(ResourceData* data)
{
	if(data == NULL)
		return;
...
void ResourceManager::Run(IThread& thread)
{
...
IResourceLoader* ResourceManager::GetLoaderFromSuffix(const std::string& suffix) const
{
