...
#define AssertEquals(Value, Expected) \
	if((Value) != (Expected)) TestApplication::AddFailure(TestCaseRunner::GetActiveTestCase(), __FILE__, __LINE__);

#define AssertNotEquals(Value, Expected) \
	if((Value) == (Expected)) TestApplication::AddFailure(TestCaseRunner::GetActiveTestCase(), __FILE__, __LINE__);
}
