...
void OGL3RenderSystem::ApplyRenderTargets()
{
	...
	OGL3StatePolicyGuard::Viewport(Rect(0, 0, size.X, size.Y));
}
...
IRenderTarget2D* OGL3RenderSystem::CreateRenderTarget2D(const Size& size, TextureFormat::Enum format)
{
	...
	OGL3StatePolicyGuard::BindTexture(GL_TEXTURE_2D, textureID);
	glTexImage2D(GL_TEXTURE_2D, 0, _internalFormat, size.X, size.Y, 0, _format, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, OGL3MinFilter[MinFilter::Default]);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, OGL3MagFilter[MinFilter::Default]);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, OGL3TextureWrap[TextureWrap::Default]);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, OGL3TextureWrap[TextureWrap::Default]);
	glFlush();
